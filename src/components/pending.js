...
function Pending(props) {
  ...
  useEffect(()=>{
    ...
    Axios.get('https://appointment0backend.herokuapp.com/pending').then((res)=>{
  ...
  const reject=(id)=>{
    ...
    return (
      ...
      <h4 id="time">Pending Requests</h4>
        
      <div className="list-group">
        {request1_list.map((val,key)=>{
          return(
            <div className="list-group-item list-group-item-action list-group-item-danger" >Name : {val.userName}   Email : {val.email} Reason : {val.reason} Time : {val.time}<button type="submit" id ="btn_req" onClick={()=>approve(val._id)} className="btn btn-success btn-sm">Approve</button>
